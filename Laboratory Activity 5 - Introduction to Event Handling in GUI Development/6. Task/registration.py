...
class myWindow:
    ...
    def submit(self):
        ...
        for field, value in entries.items():
            if not value:
                messagebox.showwarning("Input Error", f"{field} is missing!")
                return
        messagebox.showinfo("Success", "Sucessfully Submitted!")

        with open("dataEntries.txt", "a") as my_file:
            my_file.write (",".join([
            f"First Name {self.Entry2.get()}",
            f"Last Name {self.Entry3.get()}",
            f"Username {self.Entry4.get()}",
            f"Password {self.Entry5.get()}",
            f"Email {self.Entry6.get()}",
            f"Contact Number {self.Entry7.get()} \n"
            ]))
        self.clear()
